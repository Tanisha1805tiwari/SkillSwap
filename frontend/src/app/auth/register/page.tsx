    ...
    <div className="min-h-screen bg-background bg-grid flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1">Get 5 free credits on signup</p>
        </div>

        <div className="glass-card p-8">
          {/* Free credits callout */}
          <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20 mb-6">
            <div className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center flex-shrink-0">
              <span className="text-amber-400 font-bold text-sm">5</span>
            </div>
            <p className="text-sm text-amber-200">Free credits added to your account instantly</p>
          </div>
                ...
                <div className="mt-2 space-y-1">
                  {[
                    ...
                    <div key={check.key} className="flex items-center gap-1.5">
                      <CheckCircle className={`w-3 h-3 ${passwordChecks[check.key as keyof typeof passwordChecks] ? 'text-emerald-400' : 'text-slate-600'}`} />
                      <span className={`text-xs ${passwordChecks[check.key as keyof typeof passwordChecks] ? 'text-emerald-400' : 'text-muted-foreground'}`}>
                        {check.label}
                      </span>
                    </div>
                  ))}
